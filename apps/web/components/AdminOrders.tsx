  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-muted-foreground">Supervisión global de pedidos de todas las tiendas.</p>
      </div>
                  ...
                  <div key={order.id} className="grid gap-4 p-5 lg:grid-cols-[1fr_auto] lg:items-center">
                    <div className="flex min-w-0 gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2"><p className="font-black">#{order.id.slice(0, 8).toUpperCase()}</p><Badge>{order.status}</Badge>{order.payment && <Badge>{order.payment.status}</Badge>}</div>
                        <p className="mt-1 truncate font-semibold">{order.store.name} · {order.buyer.name}</p>
                        <p className="text-sm text-muted-foreground">{order.buyer.email} · {order.items.reduce((sum, item) => sum + item.quantity, 0)} unidades</p>
                        <p className="mt-1 text-xs text-muted-foreground">{new Date(order.createdAt).toLocaleString('es-UY')}</p>
                      </div>
                    </div>
